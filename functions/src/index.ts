...
app.intent('experience', async (conv: DialogflowConversation, { experience }: { experience: any }) => {
    if (experience !== null && experience !== undefined){
        ...
        if (!conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT')) {
            conv.ask(await bagCore.countMyList(conv.user.storage.days, experience));
        } else {
            conv.ask(await bagCore.countMyResponse(conv.user.storage.days, experience));
        }
        conv.close("Wish you luck on upcoming trip!");
    } else {
